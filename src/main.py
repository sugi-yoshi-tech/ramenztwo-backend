...
app = FastAPI(
    ...
    description="プレスリリース取得・分析API - 実データ版",
    version="2.0.0"
)
...
async def on_startup():
    ...
    logger.info("Startup complete - Real Data API ready")
...
async def root():
    return {
        "service": "PR TIMES RAG API (Real Data)",
        "version": "2.0.0",
        ...
        "notes": [
            "All data fetched from PR TIMES stg API",
            "Companies list with caching (5 min)",
        ],
    }
...
# ===============================
# Streamlit互換 - AI分析（OpenAI + 任意RAG文脈）
# ===============================
...
async def analyze_press_release(payload: PressReleaseInput):
    """
    プレスリリース分析エンドポイント
    - OpenAI によるメディアフック評価・改善提案
    - context_category_id があれば同カテゴリの直近期上位をRAG文脈として付与
    """
    request_id = str(uuid.uuid4())
    started = datetime.now()

    # 1) RAG文脈（任意）- 既存のコードと同じ
    ...
    if state.openai is None:
        ...
        return {
            ...
            "media_hook_evaluations": [
                {"hook_type": "trending_seasonal", "hook_name_ja": "トレンド・季節性", "score": 3, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "unexpectedness", "hook_name_ja": "意外性", "score": 2, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "paradox_conflict", "hook_name_ja": "パラドックス・対立構造", "score": 2, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "regional", "hook_name_ja": "地域性", "score": 1, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "topicality", "hook_name_ja": "話題性", "score": 3, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "social_public", "hook_name_ja": "社会性・公共性", "score": 3, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "novelty_uniqueness", "hook_name_ja": "新規性・独自性", "score": 4, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "superlative_rarity", "hook_name_ja": "最上級・希少性", "score": 2, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
                {"hook_type": "visual_impact", "hook_name_ja": "ビジュアルインパクト", "score": 3 if (payload.top_image and payload.top_image.url) else 1, "description": "OpenAI未設定のため簡易評価", "improve_examples": [], "current_elements": []},
            ],
    ...
    system_prompt = (
        ...
        "**重要**: 以下の9つのメディアフック項目について、必ずすべて評価してください。1つも欠かしてはいけません。"
        "各項目のスコアは1〜5の整数で評価し、根拠を簡潔に述べてください。"
        "出力は必ずJSON一つだけ。説明文は不要。"
    )
        ...
        "media_hook_evaluations": [
            {
                ...
                "score": 3,
                "description": "評価理由を1〜2文で説明",
                "improve_examples": ["具体的な改善案1", "具体的な改善案2"],
                "current_elements": ["現状で満たしている要素1", "現状で満たしている要素2"]
            } for hook in required_hooks
        ],
        "paragraph_improvements": [
            {"where": "タイトル/導入/本文の段落番号など", "before": "元文(抜粋)", "after": "改善案(具体的に)"}
        ],
        "overall_assessment": {
            "total_score": 0.0,
            "strengths": ["強み1", "強み2"],
            "weaknesses": ["弱み1", "弱み2"],
            "top_recommendations": ["推奨事項1", "推奨事項2"],
            "estimated_impact": "推定インパクトを短い一文で"
        }
    }
        ...
        completion = await state.openai.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": system_prompt},
                {
                    "role": "user",
                    "content": (
                        "以下の入力を分析し、9つのメディアフック項目すべてについてスコアと改善提案を出してください。"
                        "『related_context.items』は同カテゴリの参考リリース一覧です。"
                        "『required_evaluations』に示された9項目すべてを必ず評価してください。"
                        "JSON以外は出力しないでください。\n\n"
